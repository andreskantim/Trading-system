...
if __name__ == '__main__':
    ...
    *best_params, best_real_pf = result
    ...
    output_file = output_dir / 'insample_mcpt.png'
    ...
    ax.set_title(f"In-sample Cumulative Returns ({strategy_name}) | Real vs {len(perm_cum_rets)} Permutations",
                 fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=11, loc='upper left')
    plt.tight_layout()

    output_file_cum = output_dir / 'insample_cumulative_mcpt.png'
    ...
    results_csv = output_dir / f'insample_results_{slurm_config["job_id"]}.csv'
    ...
    print(f"  - insample_results_{slurm_config['job_id']}.csv")
    print("="*70 + "\n")
